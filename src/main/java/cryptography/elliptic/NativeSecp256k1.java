...
import static cryptography.elliptic.NativeSecp256k1Util.AssertFailException;
import static cryptography.elliptic.NativeSecp256k1Util.assertEquals;
import static cryptography.elliptic.NativeSecp256k1Util.checkArgument;
import static cryptography.utils.JniLoader.ABSOLUTE_PATH;
...
public final class NativeSecp256k1 {
    ...
    public static void init() {
        if (INSTANCE == null) {
            try {
                System.load(ABSOLUTE_PATH + "/libs/libsecp256k1_jni.so");
    ...
    public static boolean secKeyVerify(long ctx, byte[] secretKey) {
        checkArgument(secretKey.length == 32, "Secret length must be 32 bytes");
